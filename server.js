...
app.post("/newsletter", async (req, res) => {
    try {
        ...
        if (!email) {
            return res.status(400).json({
                success: false,
                message: "<strong>Email is Required!</strong>"
            });
        }

        const existingSubscriber = await MailingList.findOne({
            email
        });
        ...
        if (existingSubscriber) {
            return res.status(400).json({
                success: false,
                message: "<strong>❌ Email Already Subscribed!</strong>"
            });
        }
        ...
        return res.status(200).json({
            success: true,
            message: "<strong>✅ Thank You For Subscribing!</strong>"
        });
    } catch (error) {
        console.error("Newsletter Error:", error);
        return res.status(500).json({
            success: false,
            message: "<strong>An Error Occurred. Please Try Again Later. 🕒</strong>"
        });
    }
});
...
app.post("/contactus", async (req, res) => {
    try {
        ...
        res.status(200).json({
            message: "Message Sent Successfully! Wait For Us To Get Back To You ⏳"
        });
    } catch (error) {
